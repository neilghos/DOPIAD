...
def load_smap_compact(data_root: str | Path, entity_id: str = 'SMAP_Compact') -> HNNCompactEntity:
    root = Path(data_root)
    ...
    if test_points.shape[0] != test_labels.shape[0]:
        raise ValueError(f'SMAP pooled test shape/label mismatch: test={test_points.shape[0]} labels={test_labels.shape[0]}')
