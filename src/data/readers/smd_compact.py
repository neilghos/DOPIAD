...
from data.types import CompactEntityData
...
def load_smd_compact_entity(data_root: str | Path, entity_id: str = 'SMD_Compact') -> CompactEntityData:
    root = Path(data_root)
    ...
    if test_points.shape[0] != test_labels.shape[0]:
        raise ValueError(
            f'SMD pooled test shape/label mismatch: test={test_points.shape[0]} labels={test_labels.shape[0]}'
        )

    feature_names = [f'sensor_{i:02d}' for i in range(train_points.shape[1])]

    return CompactEntityData(
        entity_id=entity_id,
